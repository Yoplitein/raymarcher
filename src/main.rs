...
fn main() -> AResult<()> {
	...
	let models = {
		...
		let smallSphere = sdf::sd_sphere(0.25);
		let smallSpheres = {
			let s1 = smallSphere.clone().translate(vec3(1.0, 1.0, 1.0));
			let s2 = smallSphere.clone().translate(vec3(-1.0, 1.0, 1.0));
			let s3 = smallSphere.clone().translate(vec3(1.0, 1.0, -1.0));
			let s4 = smallSphere.clone().translate(vec3(-1.0, 1.0, -1.0));
		...
		/* [
			...
			Model::new(Rgb([255, 255, 0]), smallSpheres),
			Model::new(Rgb([255; 3]), floor),
		] */
	...
	let subpixel = vec2(1.0 / (width - 1) as f32, 1.0 / (height - 1) as f32) / 4.0;
	let sampleSteps = if args.normals { (0 ..= 0) } else { (-1 ..= 1) }.map(|v| v as f32);
...
impl Camera {
	...
	pub fn get_ray(&self, ndc: Vec2) -> Ray {
		...
		let viewportPoint = viewportCenter + self.right * ndc.x * self.aspect + self.up * ndc.y;
